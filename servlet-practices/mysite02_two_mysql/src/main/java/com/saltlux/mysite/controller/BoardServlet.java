...
import com.saltlux.mysite.dao.BoardDaoFirst;
import com.saltlux.mysite.dao.BoardDaoSecond;
...
public class BoardServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if ("writeform".equals(action)) {
			...
			BoardDaoFirst boardDaoFirst = new BoardDaoFirst();
			BoardDaoSecond boardDaoSecond = new BoardDaoSecond();
			UserVo authUser = (UserVo) session.getAttribute("authUser");

			// writeform이 문자열의 마지막이면 신규 등록
			if (referer.substring(referer.length() - 9, referer.length()).equals("writeform")) {// 신규등록
				g_no = boardDaoSecond.getMaxGno() + 1; // 자동으로 Group No를 증가한다

			} else { // 답글작성
				BoardVo vo = new BoardVo();
				Long boardNo = Long.parseLong(request.getParameter("reply"));// 답을 달 원글
				vo = boardDaoSecond.findByNo(boardNo);// 답을 달 글의 정보 요청
				...
				boardDaoFirst.updateOrder(g_no, o_no); // 추가하기 전에 원글의 뒷부분에 orderno를 늘려준다
			}
...
//			request.setAttribute("list", list);
...

		{
			...
			BoardVo vo = new BoardDaoSecond().findByNo(Long.parseLong(boardNo));
			...
			BoardDaoFirst boardDao = new BoardDaoFirst();
			BoardVo vo = new BoardVo();
			vo.setNo(delNo);
			vo.setUser_no(authUser.getNo());
			boardDao.delete(vo);
			...
			Long no = Long.parseLong(request.getParameter("no"));
			BoardVo vo = new BoardDaoSecond().findByNo(no);
			new BoardDaoFirst().updateView(no);
			...
			totalCount = boardDaoSecond.getCount();
			PagerVo pagerVo = new PagerVo(totalCount, 5, nowPage);
			list = boardDaoSecond.findAll(pagerVo);
